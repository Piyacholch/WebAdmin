    ...
    <div class="search mt-0">
      <div class="row row-menu sort">
        ...
        <div class="col col-flex d-flex justify-content-end mt-5">
          <a href="/insertchatpositive" class="a-insert">
      ...
      >
        ...
        <div class="col-3 button-1">
          <button
            ...
            @click="$router.push(`/updatechatpositive/${item.id}`)"
          >
            แก้ไข
          </button>
...
import Breadcrumb from "../../components/BC-positive.vue";
...
export default {
  ...
  methods: {
    getchatopen() {
      axios.get(process.env.VUE_APP_BACKEND_BASE_URL+"/chatpositive", { params: { sortBy: this.sortBy, first: this.first } }).then((response) => {
    ...
    deletechat(id) {
      axios.delete(process.env.VUE_APP_BACKEND_BASE_URL+"/chatpositive/" + id).then(() => {
        this.id = id;
        this.getchatopen();
