<template>
  ...
  <div class="Box pb-5">
    <div class="px-5">
      <h1>แก้ไขข้อความตอบกลับประโยคด้านบวก</h1>
      <hr />
    </div>
    <div class="mb-5 px-5">
      <Breadcrumb />
...
import Breadcrumb from "../../../components/bcupdate/bc-updatechatpositive.vue";
...
export default {
  ...
  methods: {
    getChatByID() {
      axios
        .get(`http://localhost:5050/chatpositiveByID/${this.$route.params.id}`)
        .then((response) => {
    ...
    updateChatopen() {
      axios
        .patch(
          `http://localhost:5050/updatechatPositive/${this.$route.params.id}/${this.data?.Text}`
        )
        .then((response) => {
          this.data = response.data;
